package org.fcitmuk.openclinica.forms;
...
import org.fcitmuk.openclinica.CrfDef;
import org.fcitmuk.openclinica.StudyEvent;
import org.fcitmuk.openclinica.StudyEventList;
import org.fcitmuk.openclinica.StudyRequestHeader;
import org.fcitmuk.openclinica.Subject;
import org.fcitmuk.openclinica.SubjectData;
import org.fcitmuk.openclinica.SubjectEvent;
import org.fcitmuk.openclinica.SubjectForm;
import org.fcitmuk.openclinica.VarNames;
import org.fcitmuk.openclinica.db.OpenclinicaDataStorage;
...
public class MainForm extends MIDlet  implements CommandListener,FormListener,StorageListener,AlertMessageListener, TransportLayerListener,LogonListener, Runnable{
	...
	public MainForm() {
		...
		transportLayer.setDefaultCommnucationParameter(TransportLayer.KEY_BLUETOOTH_SERVER_ID, /*"F0E0D0C0B0A000908070605040302010"*/ "F0E0D0C0B0A000908070605040301117");
		transportLayer.setDefaultCommnucationParameter(TransportLayer.KEY_HTTP_URL, "");
		transportLayer.setDefaultCommnucationParameter(TransportLayer.KEY_SMS_DESTINATION_ADDRESS,"sms://+256714380638"); //256712330386 //256782380638 "sms://+256782380638:1234"
	...
	public boolean beforeFormDataListDisplay(FormDef formDef, boolean display){
		/*if(subject != null){
			...
			if(formDataRecordId != EpihandyConstants.NULL_ID)
				formMgr.showForm(false,EpihandyConstants.DEFAULT_STUDY_ID, formDef, formDataRecordId,true,formList);
			else
	...
	private void startSubjectDownload(){
		...
		transportLayer.setCommnucationParameter(TransportLayer.KEY_HTTP_URL, transportLayer.getConnectionParameterValue(TransportLayer.CON_TYPE_HTTP, NAME_SUBJECT_DOWNLOAD_URL)+"?uname="+userMgr.getUserName()+"&pw="+userMgr.getPassword());
		transportLayer.download(comnParam, null, new ResponseHeader(), new SubjectData(), this,userMgr.getUserName(),userMgr.getPassword());		
	}
	...
	private void startStudyEventsDownload(){
		...
		comnParam.setStudyId(formMgr.getCurrentStudy().getId());
		transportLayer.setCommnucationParameter(TransportLayer.KEY_HTTP_URL, transportLayer.getConnectionParameterValue(TransportLayer.CON_TYPE_HTTP, NAME_SUBJECT_DOWNLOAD_URL)+"?uname="+userMgr.getUserName()+"&pw="+userMgr.getPassword());
		transportLayer.download(comnParam, null, new ResponseHeader(), new StudyEventList(), this,userMgr.getUserName(),userMgr.getPassword());		
	}
